...
export default function Card({ card, faceDown = false, flipping = false }) {
  const flippingClass = flipping ? ' card--flipping' : ''
 
  if (faceDown || !card) {
    return (
      <div className={`card card--back${flippingClass}`}>
  ...
  return (
    <div className={`card ${colorClass}${flippingClass}`}>
